...
- **新建日志表单精简**：主界面只留核心项（①基础 日期/标的/方向/策略/周期 · ②入出止损/仓位/账户/杠杆/保证金模式/分批买入/实时测算 · ③入出场逻辑 · ④情绪三项 · ⑤复盘 · ⑥自评），次要字段（市场/标签/买入金额U/手续费/止盈目标/大趋势/大盘环境/情绪备注）收进「高级选项」折叠区（`details.adv`，默认收起），字段 ID 未动、历史数据无影响
...
15. **止盈目标优先级坑**：`evaluate` 中若 targets 里任一 `hit=true` 且 `ratio` 之和已达 100，主出场 `exit` 会被完全忽略（`totalRatio<100&&d.exit>0` 才用主出场）。用户 0.72 做空、在 0.94 平仓却显示盈利 +265U 的根因：target 误填 `hit=true`、`actualExit=0.4`、`ratio=100` → 系统按 0.4 全平盈利。用户手动多档出场时，target 的 `actualExit`/`hit` 填错会直接盖过 exit 字段；排错先看 targets。
