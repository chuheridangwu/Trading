...
- **智能仓位控制**（工具箱 `calcSmartSize`）：固定风险百分比反推仓位（`数量=账户×风险%÷止损距离`），加止损距离 vs 1倍波动校验、保证金占比分级（≤25健康/≤50偏重/≥50危险）、**币安爆仓价算法**（逐仓含 MMR+手续费；全仓以钱包余额为担保，支持「不会强平」）、止损在爆仓价内硬约束、金字塔分批 50/30/20 建议
