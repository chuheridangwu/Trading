...
- 分批买入/补仓（`renderLots`/`syncLotsToEntry`）：多档买入价+数量（或投入金额），自动算加权均价与总数量，锁定 entry/size 字段；详情页展示每档
- **投入金额→数量**（`syncAmtToSize`，新建日志）：只填入场价 + 投入金额 U，自动算 `数量=金额×杠杆÷价`，实时联动 f_entry/f_leverage/f_amt；有分批买入档位时自动让位
- **加仓爆仓模拟器**（工具箱 `computeSim`）：账户总资金/杠杆/当前价/逐仓or全仓 + 多档「买入价+投入金额」→ 实时算每档数量、加权均价、累计保证金、逐仓各档爆仓价、全仓综合爆仓价（`加权均价-总保证金÷总数量`），含保证金占比与爆仓距离警告
- **智能仓位控制**（工具箱 `calcSmartSize`）：固定风险百分比反推仓位（`数量=账户×风险%÷止损距离`），加止损距离 vs 1倍波动校验、保证金占比分级（≤25健康/≤50偏重/≥50危险+止损失效判危险）、逐仓爆仓价止损有效性硬约束、金字塔分批 50/30/20 建议
- 新建/编辑/列表/详情/笔记/导入导出/清单/筛子/指南/工具箱
- 自动评测引擎（`evaluate`）：扣分项 + 改进建议 + 总结语

## 核心公式速查

- 保证金数量：`数量 = 金额(U) × 杠杆 ÷ 买入价`
- **全仓综合爆仓价（多头）= 加权均价 − 账户总余额 ÷ 总数量**（担保品=整个账户余额，非占用保证金！）
- 逐仓爆仓价（多头/L>1）：`买入价 × (1 − 1/杠杆)`，每档独立，单档最多亏该档保证金
- 固定风险仓位：`数量 = 账户 × 风险% ÷ |入场−止损|`
- 止损有效性（多头）：止损必须 > 逐仓爆仓价，否则先爆仓后止损
...
8. **全仓爆仓价是简化模型**：担保品 = **账户总余额**（用户填的账户资金），`爆仓价=加权均价−账户总余额÷总数量`；未计维持保证金率(MMR)/手续费，实际爆仓价略高。曾踩坑：误用"总投入保证金"当担保品导致爆仓价算得过近（如用户 10000U 账户 3×200U 20x 场景，错误 0.714 → 正确 0.125）。账户资金未填时按占用保证金计算并在界面提示偏保守。
9. **分批买入金额/数量互斥**：档位内改『数量』会删除该档 `amt`（数量手动优先）；改『金额』自动重算数量。`amt` 不持久化，编辑时反推 `金额=数量×价÷杠杆`。
10. **函数定义顺序**：`renderSimLots`/`computeSim` 在脚本加载末尾立即执行（无档直接 return），依赖 `priceFmt` 先于其定义（在 calcSwing 区域），勿把其前移。

## 下一步计划（待办，按优先级）

- [ ] **Supabase RLS 检查与加固**（最高优先，安全）
- [ ] **用户决策：仓位控制保留哪个**——工具箱『智能仓位控制 calcSmartSize』 vs 仪表盘『仓位计算器 calcPosition』，择一保留，另一移除或引导跳转
