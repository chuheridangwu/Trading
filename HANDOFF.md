...
- **投入金额→数量**（`syncAmtToSize`，新建日志）：只填入场价 + 投入金额 U，自动算 `数量=金额÷价`，实时联动 f_entry/f_leverage/f_amt；有分批买入档位时自动让位
...
> **语义约定（用户已确认，2026-08）**：模拟器/新建日志里的「投入金额 / 买入金额 U」一律指**买入的币值（名义价值）**，不是保证金、不是投入本金。固定按此，不提供切换。
